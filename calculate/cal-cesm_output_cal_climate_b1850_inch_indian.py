...
start = time.time()
path_in  =  "/home/sun/segate/model_data/b1850_inch_indian/"
path_out =  "/home/sun/data/model_data/climate/"

year_range  =  np.array([1,24],dtype=int)

f0  =  xr.open_dataset(path_in+'b1850_inch_indian_atmosphere_h1_16_12-18.nc')
...
for yyyy in range(year_range[0],year_range[1]+1):
    ...
    for f1 in os.listdir(path_in):
        if yyyy < 10:
            if "_h1_0"+str(yyyy) in f1:
                list0.append(f1)
        else:
            if "_h1_"+str(yyyy) in f1:
                list0.append(f1)
    list0.sort()
...
ds.to_netcdf(path_out+"b1850_inch_indian_atmosphere.nc")