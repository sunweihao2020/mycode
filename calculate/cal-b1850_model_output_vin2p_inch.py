...
end_path  =  '/home/sun/segate/model_data/b1850_inch/'
...
def deal_control():
    ...
    exp_name  =  ['b1850_tx_inch_o1_220824','b1850_tx_inch_o2_220830']

    namelist1 =  []
    namelist2 =  []

    for ffff in file_all:
        if (exp_name[0] in ffff) and ('cam' in ffff) and ('mom' not in ffff) and ('.b185' not in ffff):
            namelist1.append(ffff)
        elif (exp_name[1] in ffff) and ('cam' in ffff) and ('mom' not in ffff) and ('.b185' not in ffff):
            namelist2.append(ffff)

    namelist1.sort() ; namelist2.sort()
    namelist1.pop()  ; namelist2.pop()
    namelist   =   namelist1 + namelist2

    print("Now it is {}, the affirm year number is {}".format("inch",len(namelist)/365))  # affirm that the number of files is nyear
    # period range
    ...
    for yyyy in range(year_length):
        ...
        for ffff in namelist[start:end]:
            ...
            yy        =  yyyy + 1
            if yy < 10:
                new_name  =  "b1850_inch_atmosphere_h1_0" + str(yy) + "_" + ffff[(len(exp_name[0]) + len('.cam.h1.') + 5) : (len(exp_name[0]) + len('.cam.h1.') + 10)] + ".nc"
            else:
                new_name  =  "b1850_inch_atmosphere_h1_" + str(yy) + "_" + ffff[(len(exp_name[0]) + len('.cam.h1.') + 5) : (len(exp_name[0]) + len('.cam.h1.') + 10)] + ".nc"
