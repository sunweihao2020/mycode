...
for i in range(181):
    for j in range(360):
        ...
        correlation_u_LSTC[i, j] = stats.pearsonr(ls_diff[:, 3] - np.average(ls_diff[:, 3]), f3['monthly_u'][:, i, j] - np.average(f3['monthly_u'][:, i, j]))[0]
        correlation_v_LSTC[i, j] = stats.pearsonr(ls_diff[:, 3] - np.average(ls_diff[:, 3]), f3['monthly_v'][:, i, j] - np.average(f3['monthly_v'][:, i, j]))[0]
        correlation_u_OLR[i, j]  = stats.pearsonr(f1['ttr_avg_mon'][:, 3] - np.average(f1['ttr_avg_mon'][:, 3]), f3['monthly_u'][:, i, j] - np.average(f3['monthly_u'][:, i, j]))[0]
        correlation_v_OLR[i, j]  = stats.pearsonr(f1['ttr_avg_mon'][:, 3] - np.average(f1['ttr_avg_mon'][:, 3]), f3['monthly_v'][:, i, j] - np.average(f3['monthly_v'][:, i, j]))[0]
...
ncfile.attrs['description']  =  'This file includes correlation between LSTC/OLR index and global uv field (10m wind) using ERA5 data'
ncfile.attrs['script']  =  'cal_correlation_LSTC_OLR_index_on_uv_230509.py'
ncfile.to_netcdf("/home/sun/data/ERA5_data_monsoon_onset/correlation/correlation_LSTC_OLR_10wind.nc")
