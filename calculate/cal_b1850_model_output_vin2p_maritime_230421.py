...
'''
src_path  =  '/home/sun/wd_disk/b1850_maritime_output/'
end_path  =  '/home/sun/segate/model_data/b1850_maritime/atmosphere/'
...
def deal_control():
    ...
    print("Now it is {}, the affirm year number is {}".format("maritime",len(file_list)/365))  # affirm that the number of files is nyear
    # period range
    year_length =  int(len(file_list)/365)
    ...
    for yyyy in range(year_length):
        ...
        for ffff in file_list[start : end]:
            print("{} year file name is {}".format(yyyy, ffff))
            out_ds    =  cesm_vin2p(src_path,ffff,pnew)

            yy        =  yyyy + 1
            
            new_name  =  "b1850_maritime_atmosphere_vin2p_" + ffff[30:]
