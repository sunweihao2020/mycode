...
end_path  =  '/home/sun/segate/model_data/b1850_control/'
...
def deal_control():
    ...
    # Beacuse control namelist do not have 04-01-02 but have 56-01-01, so I move the last to the head
    namelist2  =  []
    namelist2.append(namelist[-1])

    namelist2  =  namelist2 + namelist[:-1]


    print("Now it is {}, the affirm year number is {}".format("control",len(namelist2)/365))  # affirm that the number of files is nyear
    # period range
    year_range  =  [4,55]
    year_length =  int(len(namelist2)/365)
    ...
    for yyyy in range(year_length):
        ...
        for ffff in namelist2[start:end]:
            print("{} year file name is {}".format(yyyy, ffff))
            out_ds    =  cesm_vin2p(src_path,ffff,pnew)

            yy        =  yyyy + 1
            if yy < 10:
                new_name  =  "b1850_control_atmosphere_h1_0" + str(yy) + "_" + ffff[(len(exp_name[0]) + len('.cam.h1.') + 5) : (len(exp_name[0]) + len('.cam.h1.') + 10)] + ".nc"
            else:
                new_name  =  "b1850_control_atmosphere_h1_" + str(yy) + "_" + ffff[(len(exp_name[0]) + len('.cam.h1.') + 5) : (len(exp_name[0]) + len('.cam.h1.') + 10)] + ".nc"
