'''
2022-7-22
This code is to interp data to plev. The experiment is b1850 control
time period: 04 - 55
...
path_out   =     '/home/sun/data/model_data/daily/'
...
for nn in file_list:
    ...
    if "global" in nn and ".b185" not in nn:
        global_1m.append(nn)
...
for name in global_1m:
    if ".cam.h1." in name:
        print("now it is deal "+name)
        out_ds  =  cesm_vin2p(path,name,pnew)
        out_ds.attrs["description"]  =  "global 1m b1850 after vin2p. 220809"
