...
def transform_t_p(t,p):
    #分别求出平均温度以及lnσ
    #这里我把顺序调过来了，自下往上
    a_t   =  ma.zeros((71,41,97))
    for ll in range(1,72):
        a_t[ll-1,:,:]  =  (t[71-ll,:,:] + t[71,:,:])/2
    p0    =  p[71,:,:]
    sigma =  copy.deepcopy(p)
    for ll in range(0,p.shape[0]):
        sigma[ll,:,:] = p[71-ll,:,:]/p0
    return a_t,np.log(1/sigma[1:,:,:])  #!!注意这里返回的是对数
...
for f1 in files:
    ...
    vt          = ma.zeros((1, 71, 41, 97))
    ut          = ma.zeros((1, 71, 41, 97))
    vt[0, :, :, :]  =  vt_1
    ut[0, :, :, :]  =  ut_1
    fout = create_nc_multiple('/data5/2019swh/data/cal_thermal/', f1, time,lev[1:], lon, lat)
