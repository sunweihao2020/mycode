...
for day in range(0,365):
    files_1998      =    [x for x in file_list if ("Daily.1998" in x)] ; files_1998.sort()
    base_array      =    xr.open_dataset(path1+files_1998[day])
    ...
    for vars in base_array.keys():
        base_array[vars].data /= 22

    base_array.to_netcdf(path3+"trmm_"+files_1998[day][15:19]+".climate.nc")
    joint_array[day, :, :] = base_array.precip.data[:, :]
