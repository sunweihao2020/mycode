...
end_path  =  '/home/sun/segate/model_data/b1850_indian/'
...
def deal_control():
    ...
    exp_name  =  ['b1850_tx_indian_o1_220808']

    namelist  =  []

    # here I abandon the 43 year
    for ffff in file_all:
        if (exp_name[0] in ffff) and ('cam' in ffff) and ('mom' not in ffff) and ('.0043-' not in ffff):
    ...
    for yyyy in range(year_length):
        ...
        for ffff in namelist[start:end]:
            ...
            yy        =  yyyy + 1
            if yy < 10:
                new_name  =  "b1850_indian_atmosphere_h1_0" + str(yy) + "_" + ffff[(len(exp_name[0]) + len('.cam.h1.') + 5) : (len(exp_name[0]) + len('.cam.h1.') + 10)] + ".nc"
            else:
                new_name  =  "b1850_indian_atmosphere_h1_" + str(yy) + "_" + ffff[(len(exp_name[0]) + len('.cam.h1.') + 5) : (len(exp_name[0]) + len('.cam.h1.') + 10)] + ".nc"
