...
@nb.jit()
def cal_modelpv(v,p,theta,u,f,disx,location,vp,up,thetap,vx,thetax):
    #计算垂直差分项
...
for ff in file_theta:
    ...
    pvh, pvv, all_pv = cal_modelpv(v,p,theta,u,corios,disx,location,vp,up,thetap,vx,thetax)
    time = np.arange(0, 1) + 1
    file = create_nc_multiple(path2, "model_pv_" + ff[12:20], time, lev, lon, lat)
