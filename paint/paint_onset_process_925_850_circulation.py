...
def paint_pic(extent,sst_lon,sst_lat,wind_lon,wind_lat,prect_lat,prect,sst,U2M,V2M,date,dates,number):
    ...
    for col in range(2):
        for row in range(2):
            ...
            # 等值线
            im1  =  ax.contour(sst_lon,prect_lat,prect[date[j],:],levels=np.arange(12,35,6),colors='#6dc686',linewidths=3,alpha=1,zorder=1)
            im2  =  ax.contourf(sst_lon,sst_lat,sst[date[j],:],levels=np.linspace(25,31,13),cmap=cmap,extend='both',zorder=0)
            ...
            # 流线
            q   =   ax.streamplot(wind_lon, wind_lat, U2M[date[j],:], V2M[date[j],:],linewidth=3, color = 'k',density=[1, 1.15], arrowsize=1.5, arrowstyle='->')
    ...
    # 保存图片
    #plt.tight_layout()
    save_fig(path_out="/home/sun/paint/lunwen/version3.0/",file_out="lunwen_fig3_v3.0_prect_sst_850wind.pdf")
...
def main():
    ...
    f3    =  xr.open_dataset(path+"composite3.nc").sel(lon=lon_slice,lat=lat_slice,level=850)
    ...
    #  下面开始绘图
    paint_pic(extent=extent,sst_lon=f1.lon,sst_lat=f1.lat,wind_lon=f3.lon,wind_lat=f3.lat,prect=trmm_prect,sst=f1.sst.data,U2M=f3.uwind.data,V2M=f3.vwind.data,date=date,dates=dates,number=number,prect_lat=np.linspace(-50,50,101))
