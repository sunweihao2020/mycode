...
input_list.add_argument('--sst_avg', '-sst_a', help='SST file for climate average', default='surface_sensible_heat_flux_climatic_daily.nc')
input_list.add_argument('--sst_early', '-sst_e', help='SST file for early onset year', default='surface_sensible_heat_flux_climatic_daily_year_early.nc')
input_list.add_argument('--sst_late',  '-sst_l', help='SST file for late onset year', default='surface_sensible_heat_flux_climatic_daily_year_late.nc')
input_list.add_argument('--figname', '-n', help='name of the picture1', default='3-5_sshf_deviation.pdf')
input_list.add_argument('--title', '-t', help='title', default='')
args        =  input_list.parse_args()

# --------------- Other information ------------------------------------------
month_name  =  ['March', 'April', 'May']
clev        =  np.linspace(-15, 15, 11)
...
j = 0
for i in range(3):
    deviation = -1 * np.average(sst_file2['sshf'].data[day_slice[i] : day_slice[i+1]], axis=0) - ( -1 * np.average(sst_file1['sshf'].data[day_slice[i] : day_slice[i+1]], axis=0))
    ...
    # Contour
    im1  =  ax.contour(ref_file0['lon'].data, ref_file0['lat'].data, deviation / 86400 * 24, levels=clev, colors='black', linewidths=1.5, alpha=1, zorder=1)
    # Contourf
    im2  =  ax.contourf(ref_file0['lon'].data, ref_file0['lat'].data, deviation / 86400 * 24, levels=clev, cmap=cmap, extend='both', zorder=0)
...
j = 0
for i in range(3):
    deviation = -1 * np.average(sst_file3['sshf'].data[day_slice[i] : day_slice[i+1]], axis=0) - ( -1 * np.average(sst_file1['sshf'].data[day_slice[i] : day_slice[i+1]], axis=0))
