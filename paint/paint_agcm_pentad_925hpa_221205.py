...
def paint_diff_pentad(agcm_u,cgcm_u,agcm_v,cgcm_v,extent=[45,115,-10,30]):
    ...
    # For the intepolate error, CGCM output has abnormal value. But AGCM output is right
    cgcm_u_data = cgcm_u.data
    cgcm_v_data = cgcm_v.data
    for i in range(len(pentads)):
        for j in range(192):
            for k in range(288):
                if np.isnan(agcm_u.data[i,j,k]):
                    cgcm_u_data[i,j,k] == 0
                    cgcm_v_data[i,j,k] == 0
                else:
                    continue
    cgcm_v_data[cgcm_v_data > 20]   =  0
    cgcm_u_data[cgcm_u_data > 20]   =  0
    ...
    for row in range(2):
        for col in range(3):
            ...
            #print(agcm_u[j])
            # vector map
            q  =  ax.quiver(f0.lon, f0.lat, cgcm_u_data[j], cgcm_v_data[j], 
                regrid_shape=15, angles='uv',   # regrid_shape这个参数越小，是两门就越稀疏
                scale_units='xy', scale=1.0,        # scale是参考矢量，所以取得越大画出来的箭头就越短
                units='xy', width=0.25,
                transform=proj,
    ...
    save_fig(path_out="/home/sun/paint/b1850_exp/f2000_ensemble/compare_with_cgcm/",file_out="Pentad_agcm_925wind.pdf")
