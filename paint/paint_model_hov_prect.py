...
def lon_avg_prect(time_slice=slice(100,201),lat_slice=slice(10,25),lon_slice=slice(65,130)):
    ...
    noicid_prect    =  xr.open_dataset("/home/sun/model_output/replace_sealand_indian_vin2p/average/replace_marin_indian_climate.nc").sel(time=time_slice,lat = lat_slice,lon = lon_slice)
...
def paint_model_hov(model,path_out,filename,time_slice=slice(100,201),lat_slice=slice(10,25),lon_slice=slice(65,130)):
    '''本函数绘制模式的四个敏感性实验结果'''
    ...
    j = 0
    name   =  ["CTRL","NO_INCH","NO_INDO","NO_I\n&NO_INDO"]
...
def main():
    ...
    paint_trmm_hov(trmm,path_out="/home/sun/paint/model_out/",filename="trmm_hov_prect.pdf")
    paint_model_hov(model,path_out="/home/sun/paint/model_out/",filename="model_hov_prect_ctrl_india_indo_indianandsealand.pdf")
