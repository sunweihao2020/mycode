...
data is from b1850 experiments
'''
path0  =  "/home/sun/data/model_data/climate/"
...
def paint_jan_jul_tem_stream():
    ...
    # set range
    lon_slice  =  slice(90,97)

    f0   =  xr.open_dataset(path0 + "b1850_control_atmospheric_monthly_average.nc").sel(lon=lon_slice)
    ...
    for row in range(1):
        for col in range(3):
            ...
            ax2.streamplot(f0.lat.data, new_level[::-1], v[month[j], ::-1], w[month[j], ::-1], color='k', linewidth=2.5,
                           density=[2.5,2.5], arrowsize=2.75, arrowstyle='->')
            ax2.set_yticks([])
    ...
    plt.savefig("/home/sun/paint/b1850_exp/monthly/Jan_and_Jul_temperature_gradient_streamline_b1850_control.pdf",dpi=500)
    plt.show()
