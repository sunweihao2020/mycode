...
def paint_zonal_section(merra,control):
    ...
    # plot vwind
    im  =  ax.contourf(file.lon.data,file.lev.data,merra,levels = np.linspace(-7,7,15),cmap='coolwarm',extend='both')
...
def main():
    ...
    merra_v   =  average_latitude_month(f=file,lat_slice=slice(-2.5,2.5), time_slice=slice(6,8),lon_slice=slice(30,165),varname='merra_v')
    control_v = average_latitude_month(f=file, lat_slice=slice(-2.5,2.5), time_slice=slice(6,8),lon_slice=slice(30,165), varname='control_v')
