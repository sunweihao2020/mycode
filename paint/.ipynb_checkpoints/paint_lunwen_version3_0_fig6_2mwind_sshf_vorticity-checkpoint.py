...
def paint_pic(extent,f2_lon,f2_lat,sh,f1_lon,f1_lat,vorticity,u,v):
    ...
    for col in range(2):
        for row in range(2):
            ...
            im2  =  ax.contour(f1_lon,f1_lat,1e6*(vorticity[number_date.date[j],:]),np.linspace(-10,10,6),colors='b',alpha=0.9,linewidths=2.5)
            ax.clabel(im2, np.linspace(-10,10,6).astype(int), inline=True, fontsize=12)
            ...
            # 加日期
            add_text(ax,string="D"+str(number_date.dates[j]),location=(0.85,0.9),fontsize=25)
            # 加序号
            add_text(ax=ax,string="("+number_date.number[j]+")",location=(0.05,0.9),fontsize=25)
            j += 1
