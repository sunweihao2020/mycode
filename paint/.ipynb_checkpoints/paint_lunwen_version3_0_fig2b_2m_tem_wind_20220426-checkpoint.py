...
def set_cartopy_tick(ax, extent, xticks, yticks, nx=0, ny=0,
    xformatter=None, yformatter=None,labelsize=15):
...
def paint_picture(lon,lat,avg_t2m,avg_u,avg_v):
    # 绘制图像
    proj    =  ccrs.PlateCarree()
    ...
    # 加序号
    plv3_2a.add_text(ax=ax,string="(b)",fontsize=27.5)
    ...
    a.ax.set_label(family='serif')
    a.ax.tick_params(labelsize=15)

    # 保存图片
    save_fig(path_out="/home/sun/paint/lunwen/version3.0/",file_out="lunwen_fig2b_v3.0_2m_tem_wind.pdf")
...
def main():
    ...
    paint_picture(lon=f1.lon,lat=f1.lat,avg_t2m=avg_t2m,avg_u=avg_u,avg_v=avg_v)
