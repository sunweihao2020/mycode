...
def paint_diff_pentad(agcm_ts, cgcm_ts, extent=[45,115,-10,30]):
    ...
    for row in range(2):
        for col in range(3):
            ...
            # shade - TS
            im2  =  ax.contourf(f0.lon.data, f0.lat.data, agcm_ts[j],  51,cmap='coolwarm',alpha=1,extend='both')
            #im3  =  ax.contour(f0.lon.data, f0.lat.data,  agcm_ts[j],  np.linspace(20,35,31), colors='black',linewidths=2,alpha=1,zorder=1)
            ...
            ax.set_title('AGCM', loc='right', fontsize=25)

            j += 1

    save_fig(path_out="/home/sun/paint/b1850_exp/f2000_ensemble/compare_with_cgcm/",file_out="Pentad_agcm_TS.pdf")
