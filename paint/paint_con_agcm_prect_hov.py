...
def lon_avg_prect(time_slice=slice(100,201),lat_slice=slice(10,20),lon_slice=slice(65,130)):
    ...
    agcm_prect  =  xr.open_dataset(data_path0 + 'PRECT_climate2.nc').sel(time=time_slice,lat = slice(12,20),lon = lon_slice)
    cgcm_prect  =  xr.open_dataset(data_path1 + 'b1850_control_climate_atmosphere.nc').sel(time=time_slice,lat = lat_slice,lon = lon_slice)

    # calculate weight average
    avg_cgcm_prect  =  (cgcm_prect["PRECT"]*(np.cos(np.deg2rad(cgcm_prect.lat.values)))[None, :, None]).sum(dim='lat')/np.sum(np.cos(np.deg2rad(cgcm_prect.lat.values)))*86400*1000
    avg_agcm_prect  =  (agcm_prect["PRECT"]*(np.cos(np.deg2rad(agcm_prect.lat.values)))[None, :, None]).sum(dim='lat')/np.sum(np.cos(np.deg2rad(agcm_prect.lat.values)))*86400*1000
...
def main():
    model = lon_avg_prect()
    paint_model_hov(model,path_out="/home/sun/paint/b1850_exp/f2000_ensemble/",filename="hov_10-20_con_agcm_cgcm.pdf")
    paint_model_hov_diff(model,path_out="/home/sun/paint/b1850_exp/f2000_ensemble/",filename="hov_10-20_con_agcm_cgcm_diff.pdf")
