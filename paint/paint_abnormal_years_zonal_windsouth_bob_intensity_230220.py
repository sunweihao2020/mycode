...
file0 = '10m_u_component_of_wind_climatic_daily.nc'
file1 = '10m_u_component_of_wind_climatic_daily_year_early.nc'
file2 = '10m_u_component_of_wind_climatic_daily_year_late.nc'
...
def paint_early_late_cef_intensity(early_cef, late_cef):
    ...
    # y-axis
    y_tick = np.linspace(-3, 3, 16)

    # 3.2 Plot the early year using blue color
    plt.bar(ref_file['time'].data, early_cef, color='blue')

    # 3.3 Plot the late year using red color
    plt.bar(ref_file['time'].data, late_cef, color='red')
