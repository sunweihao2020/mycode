...
Variables: 925 hPa wind, prect, omega
'''
...
def paint_diff_pentad(agcm_u, cgcm_u, agcm_v, cgcm_v, agcm_omega, cgcm_omega, agcm_prect, cgcm_prect, extent=[45,115,-10,30]):
    ...
    for row in range(2):
        for col in range(3):
            ...
            # shade - prect
            im2  =  ax.contourf(f0.lon.data, f0.lat.data, (cgcm_prect[j] - agcm_prect[j]) * 86400000, np.linspace(-5,5,11),cmap='coolwarm_r',alpha=1,extend='both')

            # contour line - omega
            im1  =  ax.contour(f0.lon.data, f0.lat.data, (-1 * cgcm_omega[j]) - (-1 * agcm_omega[j]), np.linspace(-0.04,0.04,11),colors='red',linewidths=2,alpha=1,zorder=1)
            im3  =  ax.contour(f0.lon.data, f0.lat.data, (-1 * cgcm_omega[j]) - (-1 * agcm_omega[j]),  [0],colors='red',linewidths=3,alpha=1,zorder=1)
    ...
    save_fig(path_out="/home/sun/paint/b1850_exp/f2000_ensemble/compare_with_cgcm/",file_out="Pentad_diff_agcm_cgcm_925wind_omega_prect.pdf")
...
def main():
    ...
    # 3. read AGCM OMEGA
    f3   =  xr.open_dataset(path1 + 'OMEGA_pentad.nc').sel(lev = 700, time = pentads)

    # 4. read AGCM PRECT
    f4   =  xr.open_dataset(path1 + 'PRECT_pentad.nc').sel(time = pentads)

    # 3. send to paint pic
    paint_diff_pentad(f0.U, f2.sel(lev=925).U, f1.V, f2.sel(lev=925).V, f3.OMEGA, f2.sel(lev=700).OMEGA, f4.PRECT, f2.PRECT)
