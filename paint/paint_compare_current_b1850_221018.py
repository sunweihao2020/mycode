...
same with paint_compare_current_b1850_221012.py but for April compare
'''
import os
...
#   ncfile.to_netcdf('/home/sun/data/model_assessment/low-resolution-merra2_b1850_control_wind.nc')
    ...
    im2 = ax.contourf(f0.lon, f0.lat, f0.speed_merra2.data[3], levels=clev, cmap=newcmp, extend='both',
                      zorder=0)

    # vector for wind
    q = ax.quiver(f0.lon, f0.lat, f0.merra_u.data[3], f0.merra_v.data[3],
    ...
    # contourf for jet , bigger than 10 is painted
    clev = np.linspace(10, 20, 5)
    im2 = ax.contourf(f0.lon, f0.lat, f0.speed_model.data[3], levels=clev, cmap=newcmp,
                      extend='both',
                      zorder=0)

    # vector for wind
    q = ax.quiver(f0.lon, f0.lat, f0.control_u.data[3], f0.control_v.data[3],
                  regrid_shape=15, angles='uv',  # regrid_shape这个参数越小，是两门就越稀疏
                  scale_units='xy', scale=1,  # scale是参考矢量，所以取得越大画出来的箭头就越短
                  units='xy', width=0.25,
                  transform=proj,
    ...
    # save picture
    save_fig(path_out="/home/sun/paint/b1850_exp/assessment/", file_out="compare_current_b1850_850wind_april.pdf")
    ...
    # contourf for jet , bigger than 10 is painted
    clev = np.linspace(10, 20, 5)
    im2 = ax.contourf(f0.lon, f0.lat, f0.speed_merra2.data[3] - f0.speed_model.data[3], levels=np.linspace(-6,6,7), cmap='coolwarm',
                      extend='both',
                      zorder=0)

    # vector for wind
    q = ax.quiver(f0.lon, f0.lat, f0.merra_u.data[3] - f0.control_u.data[3], f0.merra_v.data[3] - f0.control_v.data[3],
                  regrid_shape=15, angles='uv',  # regrid_shape这个参数越小，是两门就越稀疏
                  scale_units='xy', scale=1.,  # scale是参考矢量，所以取得越大画出来的箭头就越短
                  units='xy', width=0.25,
                  transform=proj,
    ...
    save_fig(path_out="/home/sun/paint/b1850_exp/assessment/", file_out="compare_current_b1850_850wind_difference_april.pdf")
