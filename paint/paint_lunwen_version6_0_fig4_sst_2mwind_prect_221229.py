...
def paint_pic(extent,sst_lon,sst_lat,wind_lon,wind_lat,prect_lat,prect,sst,U2M,V2M,date,dates,number):
    ...
    for col in range(2):
        for row in range(2):
            ...
            # 流线
            q   =   ax.streamplot(wind_lon, wind_lat, U2M[date[j],:], V2M[date[j],:],linewidth=2.5, color = 'k',density=[1, 1.15], arrowsize=2.75, arrowstyle='->',)
    ...
    # 保存图片
    #plt.tight_layout()
    save_fig(path_out="/home/sun/paint/lunwen/version6.0/",file_out="lunwen_fig4_v6.0_prect_sst_2mwind.pdf")
