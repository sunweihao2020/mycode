...
input_list.add_argument('--sst_avg', '-sst_a', help='SST file for climate average', default='sea_surface_temperature_climatic_daily.nc')
input_list.add_argument('--sst_early', '-sst_e', help='SST file for early onset year', default='sea_surface_temperature_climatic_daily_year_early.nc')
input_list.add_argument('--sst_late',  '-sst_l', help='SST file for late onset year', default='sea_surface_temperature_climatic_daily_year_late.nc')
input_list.add_argument('--figname', '-n', help='name of the picture1', default='3-5_sst_deviation.pdf')
...
j = 0
for i in range(3):
    deviation = np.average(sst_file2['sst'].data[day_slice[i] : day_slice[i+1]], axis=0) - np.average(sst_file1['sst'].data[day_slice[i] : day_slice[i+1]], axis=0)
...
j = 0
for i in range(3):
    deviation = np.average(sst_file3['sst'].data[day_slice[i] : day_slice[i+1]], axis=0) - np.average(sst_file1['sst'].data[day_slice[i] : day_slice[i+1]], axis=0)
