...
def paint_pic(extent,lon,lat,q1_avg,uwind,vwind):
    ...
    for col in range(2):
        for row in range(2):
            ...
            if number_date.dates[j]<0:
                add_text(ax=ax,string="D"+str(number_date.dates[j]),location=(0.90,0.91),fontsize=30)
            elif number_date.dates[j]>0:
                 add_text(ax=ax,string="D+"+str(number_date.dates[j]),location=(0.88,0.91),fontsize=30)
            else:
                 add_text(ax=ax,string="D"+str(number_date.dates[j]),location=(0.90,0.91),fontsize=30)
            # 加序号
    ...
    save_fig(path_out="/home/sun/paint/lunwen/version3.0/",file_out="lunwen_fig4_v3.0_q1_700_stream.pdf")
