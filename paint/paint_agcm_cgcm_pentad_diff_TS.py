...
def paint_diff_pentad(agcm_ts, cgcm_ts, extent=[45,115,-10,30]):
    ...
    for row in range(2):
        for col in range(3):
            ...
            # shade - TS
            im2  =  ax.contourf(f0.lon.data, f0.lat.data, (cgcm_ts[j] - agcm_ts[j]),  np.linspace(-1,1,11),cmap='coolwarm',alpha=1,extend='both')
            im3  =  ax.contour(f0.lon.data, f0.lat.data,  (cgcm_ts[j] - agcm_ts[j]),  np.linspace(-1,1,11), colors='black',linewidths=2,alpha=1,zorder=1)
    ...
    save_fig(path_out="/home/sun/paint/b1850_exp/f2000_ensemble/compare_with_cgcm/",file_out="Pentad_diff_agcm_cgcm_TS.pdf")
...
def main():
    ...
    lev      =  925
    # 1. read AGCM TS
    f1   =  xr.open_dataset(path1 + 'TS_pentad.nc').sel(time = pentads)

    # 2. read CGCM TS
    f2   =  xr.open_dataset(path0 + 'b1850_control_atmosphere_pentad.nc').sel(time = pentads)

    # 3. send to paint pic
    paint_diff_pentad(f1.TS, f2.TS)
