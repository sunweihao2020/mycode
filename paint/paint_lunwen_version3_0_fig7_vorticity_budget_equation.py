...
def paint_dvdt(extent,lon,lat,d_vor_dt):
    ...
    for col in range(2):
        ...
        # 等值线
        im1  =  ax.contour(lon,lat,d_vor_dt[number_date.date[j]]/86400*1e10,[0],linewidths=1.8,colors='k')  # 这里我除以86400因为原本的是一天的涡度变化量
        im2  =  ax.contourf(lon,lat,d_vor_dt[number_date.date[j]]/86400*1e10,np.linspace(-0.8,0.8,17),cmap=create_ncl_colormap("/home/sun/data/color_rgb/GMT_polar.txt",20),alpha=1,extend='both')
    ...
    save_fig(path_out="/home/sun/paint/lunwen/version3.0/",file_out="lunwen_fig7_v3.0_dv_dt_1.pdf")
...
def paint_other_term(extent,ncfile):
    ...
    for col in range(2):
        ...
        ax.plot([40,120],[0,0],'k--')
        # 等值线
        im1  =  ax.contour(ncfile.lon,ncfile.lat,ncfile.f_div[number_date.date[j]]*-1e5,[0],linewidths=1.8,colors='k')
        im2  =  ax.contourf(ncfile.lon,ncfile.lat,ncfile.f_div[number_date.date[j]]*-1e5,np.linspace(-3,3,13),cmap=create_ncl_colormap("/home/sun/data/color_rgb/GMT_polar.txt",23),alpha=1,extend='both')
        ax.coastlines(resolution='110m',lw=1.35)
        # 加日期
        add_text(ax,string="D"+str(number_date.dates[j]),location=(location.loc_b,location.height),fontsize=25)
        # 加序号
        add_text(ax=ax,string="("+number_date.number2[j]+")",location=(location.loc_a,location.height),fontsize=25)
        j += 1
    ...
    for col in range(2):
        ...
        ax.plot([40,120],[0,0],'k--')
        # 等值线
        im1  =  ax.contour(ncfile.lon,ncfile.lat,ncfile.v_vor_m_abs_vor_gradient[number_date.date[j]]*-1e5,[0],linewidths=1.8,colors='k')
        im2  =  ax.contourf(ncfile.lon,ncfile.lat,ncfile.v_vor_m_abs_vor_gradient[number_date.date[j]]*-1e5,np.linspace(-3,3,13),cmap=create_ncl_colormap("/home/sun/data/color_rgb/GMT_polar.txt",23),alpha=1,extend='both')
        ax.coastlines(resolution='110m',lw=1.35)
        # 加日期
        add_text(ax,string="D"+str(number_date.dates[j]),location=(location.loc_b,location.height),fontsize=25)
        # 加序号
        add_text(ax=ax,string="("+number_date.number2[j+2]+")",location=(location.loc_a,location.height),fontsize=25)
        j += 1
    ...
    for col in range(2):
        ...
        ax.plot([40,120],[0,0],'k--')
        # 等值线
        im1  =  ax.contour(ncfile.lon,ncfile.lat,ncfile.v_div_m_abs_vor_gradient[number_date.date[j]]*-1e5,[0],linewidths=1.8,colors='k')
        im2  =  ax.contourf(ncfile.lon,ncfile.lat,ncfile.v_div_m_abs_vor_gradient[number_date.date[j]]*-1e5,np.linspace(-3,3,13),cmap=create_ncl_colormap("/home/sun/data/color_rgb/GMT_polar.txt",23),alpha=1,extend='both')
        ax.coastlines(resolution='110m',lw=1.35)
        # 加日期
        add_text(ax,string="D"+str(number_date.dates[j]),location=(location.loc_b,location.height),fontsize=25)
        # 加序号
        add_text(ax=ax,string="("+number_date.number2[j+4]+")",location=(location.loc_a,location.height),fontsize=25)
        j += 1
    ...
    save_fig(path_out="/home/sun/paint/lunwen/version3.0/",file_out="lunwen_fig7_v3.0_dv_dt_2.pdf")
